<form method="post" action="{{ route('new-article') }}">
    ...
    <div class="flex flex-col w-full max-w-full p-2">
        <label for="title" class="text-xl p-1">Slug</label>
        <input id="title" type="text" name="slug" class="font-bold mt-1 focus:ring-green-500 focus:border-green-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" placeholder="Enter a spicy kekab-cased slug." required>
        @if ($errors->has('slug'))
            <span class="font-medium tracking-wide text-red-500 text-xs mt-1 ml-1">{{ $errors->first('slug') }}</span>
        @endif
    </div>
    <div class="flex flex-col w-full max-w-full p-2">
